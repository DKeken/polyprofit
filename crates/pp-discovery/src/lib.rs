...
pub async fn discover(state: &Arc<AppState>, assets: &[Asset]) -> Result<usize> {
    ...
    for q in &queries {
        // Always pass `closed=false` explicitly: as of the 2026-04 Gamma update
        // the default for `closed` flipped, and the keyword search was changed to
        // include closed markets unless we opt out.
        let url = format!(
